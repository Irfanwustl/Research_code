        ...
        for cell in self.celltypes:
            ...
            for index, prow in positiverows.iterrows():
                if len(prow['acceptedCpG']) >= self.minCpGforpositivecall:
                    if 'minfraglength' in self.kwargs:
                        if prow['fraglength'] < self.kwargs['minfraglength'][loopcounter]:
                            
                            continue

                    if 'maxfraglength' in self.kwargs:
                        if prow['fraglength'] > self.kwargs['maxfraglength'][loopcounter]:
