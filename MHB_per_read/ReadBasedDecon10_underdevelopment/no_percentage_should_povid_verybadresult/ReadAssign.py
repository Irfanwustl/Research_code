...
class ReadAssign:
    ...
    def CoreAlgo(self):
        ...
        masterdf=pd.DataFrame([[rfromrobjlist.ReadName,rfromrobjlist.allcelltype, rfromrobjlist.ucelltype, rfromrobjlist.assinedcelltype,rfromrobjlist.checkedCpG,rfromrobjlist.mismatchCpG,rfromrobjlist.notacceptedCpG,rfromrobjlist.acceptedCpG,rfromrobjlist.finalacceptedfor,rfromrobjlist.finalrejectedfor,rfromrobjlist.matefound,rfromrobjlist.hypolist, rfromrobjlist.lenhypolist, rfromrobjlist.hyperlist, rfromrobjlist.lenhyperlist, rfromrobjlist.allnotmatched, rfromrobjlist.lenallnotmatched, rfromrobjlist.fraglength,rfromrobjlist.interofhypohyper,rfromrobjlist.leninterofhypohyper] for rfromrobjlist in self.readObjectList],
                              columns=['ReadName','allcelltype', 'ucelltype', 'assignedcelltype','CheckedCpG','mismatchCpG','notacceptedCpG','acceptedCpG','finalacceptedfor','finalrejectedfor','MateCIGARok','hypolist', 'len_hypolist', 'hyperlist', 'len_hyperlist', 'allnotmatched', 'len_allnotmatched', 'fraglength','intersection_ofhypohyper','len_intersection_ofhypohyper'])
    ...
    def read_SMcpg_associate(self, smsubset, read):
        ...
        if len(result)==0:
            ...
            if len(passedCpGOnly) < 1: # self.pattern_percentage * len(checkedCpG):   ##### only  WORKS for one cell type in the SM ###############################################################
                result = []
                readobject = aRead.aRead(read.query_name, allcellsflatlistforabscountflat_list, uniquecelltypes, "NotAssigned",checkedCpG,mismatchCpG,notaccepedCpG, [])
            else:
        ...
        readobject.setextrainfo(hypolist, len_hypolist, hyperlist, len_hyperlist, allnotmatched, len_allnotmatched, fraglength)
    ...
    def collectextrainfo(self, read,offset):
        ...
        fraglength = read.template_length
