...
ggplot(to_bind,aes(x=PC1,y=PC2,shape=col_str))+geom_point(size=3)+scale_shape_manual(values=seq(65,88))+scale_color_brewer(type='qual', palette='Set1')+scale_fill_brewer(type='qual',palette="Set1")+guides(shape=guide_legend("Cell Type",ncol =1)) +theme(legend.title = element_text(size = 10),legend.text = element_text(size = 7,face="bold"))+xlim(-2.5,0)+ylim(-3,3)
