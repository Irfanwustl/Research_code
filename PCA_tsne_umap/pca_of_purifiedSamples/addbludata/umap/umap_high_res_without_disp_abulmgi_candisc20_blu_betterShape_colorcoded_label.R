...
length(namecolor)
ggplot(to_bind,aes(x=UMAP1, y=UMAP2,shape=col_str,label=rownames(to_bind)))+geom_text(hjust=2, vjust=0,size=2)+scale_shape_manual(values=seq(65,88))+guides(shape=guide_legend("Cell Type",ncol =1))+geom_point(size=3)#+theme(legend.title = element_text(size = 10),legend.text = element_text(size = 7,face="bold"))
