...
for (i in 1:length(TYPE))
{
  ...
  if(b)
  {
    if (str_detect(TYPE[i],fixed(".bw")))
    {
      ...
      col_str[i]<-sub(pattern = "(.*)\\..*$", replacement = "\\1", basename(col_str_tmp))
    }
    else
    {
...
ggplot(to_bind,aes(x=PC1,y=PC2,shape=col_str,label=rownames(to_bind)))+geom_text(hjust=2, vjust=0,size=2)+geom_point(size=3)+scale_shape_manual(values=seq(65,88))+scale_color_brewer(type='qual', palette='Set1')+scale_fill_brewer(type='qual',palette="Set1")+guides(shape=guide_legend("Cell Type",ncol =1)) +theme(legend.title = element_text(size = 10),legend.text = element_text(size = 7,face="bold"))
