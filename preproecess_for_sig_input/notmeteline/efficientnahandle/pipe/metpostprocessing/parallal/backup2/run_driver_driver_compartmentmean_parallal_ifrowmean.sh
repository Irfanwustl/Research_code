...
do
	...
	echo now=========${dirList[i]} 
	bedtools intersect -wa -a ${imputedfile} -b ${metoutfolder}/${dirList[i]} -header > ${imputedfilebgdirectory}/${dirList[i]}_bg
	...
	python3 compartmentmean_parallal.py ${imputedfilebgdirectory}/${dirList[i]}_bg_cin.txt ${metoutfolder}/${dirList[i]} ${phenfile} ${outdir}/${dirList[i]}
