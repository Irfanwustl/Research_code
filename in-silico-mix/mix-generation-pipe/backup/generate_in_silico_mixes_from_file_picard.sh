...
while getopts i:a:o:r:t:s:m: option
do
case "${option}"
in
i ) input_file=${OPTARG};;
...
do
	...
	then
		...
		dspi=(`echo "${pcts[i]}/100*$nreads/${rcount[i]}" | bc -l`)
		...
		java -jar /home/ialahi/installsoftware/picard/picard.jar DownsampleSam I=${output_merged} O=${output_downsampled} STRATEGY=HighAccuracy P=${dspi} ACCURACY=0.0000001
    ...
    fnout="insilmix_NR_$nreads"
    for (( i=0; i<$ntypes; i++ ))
    do 
	fnout=$fnout\_${indirs[i]}_${pcts[i]}
    done
    fnout=$fnout.bam
